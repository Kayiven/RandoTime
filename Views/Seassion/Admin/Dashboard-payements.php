...
// Fetch all comptes for admin
$stmt = $pdo->query("SELECT id, nom, prenom, localisation, payment_method,	price, status, created_at FROM payments");
$Payemet = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<title>Dashboard Payements</title>
...
<div class="dashboard-wrapper">
  <div class="sidebar">
    ...
    <div class="submenu">
      ...
      <a href="./Dashboard-payements.php" class="active">View Payements</a>
    </div>
    <div class="category active">Creation</div>
    <div class="submenu">
      <a href="./Dashboard-CreateData.php">Create Database</a>
    </div>
  ...
  <!-- ✅ Table next to sidebar -->
  <div class="dashboard-content">
    <h2 class="title-db">👥 Liste des Payements</h2>
    ...
    <div class="table-wrapper">
      <table class="dashboard-table">
        ...
        <tbody>
          <?php if (!empty($Payemet)): ?>
            <?php foreach ($Payemet as $Payemet): ?>
              <tr>
                <td><?= htmlspecialchars($Payemet['id']) ?></td>
                <td><?= htmlspecialchars($Payemet['nom']) ?></td>
                <td><?= htmlspecialchars($Payemet['prenom']) ?></td>
                <td><?= htmlspecialchars($Payemet['localisation']) ?></td>
                <td><?= htmlspecialchars($Payemet['payment_method']) ?></td>
                <td><?= htmlspecialchars($Payemet['price']) ?></td>
                <td><?= htmlspecialchars($Payemet['status']) ?></td>
                <td><?= htmlspecialchars($Payemet['created_at']) ?></td>
              </tr>
            <?php endforeach; ?>
          <?php else: ?>
            <tr><td colspan="6" style="text-align:center;">Aucun Contacts trouvé.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
